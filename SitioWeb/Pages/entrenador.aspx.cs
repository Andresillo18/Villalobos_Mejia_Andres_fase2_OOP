...
namespace SitioWeb.Pages
{
    public partial class entrenador : System.Web.UI.Page
    {
        ...
        {
            BLEntrenador logica = new BLEntrenador(clsConfiguracion.getConnectionString); // Se hace una instancia de la capa lógica
            DataSet DSEntrenador;

            try
            {
                DSEntrenador = logica.listarEntrenadores(condicion, orden);
                if (DSEntrenador != null)
                {
                    grdEntrenadores.DataSource = DSEntrenador;
                    grdEntrenadores.DataMember = DSEntrenador.Tables[0].TableName; // la primera tabla devuelta

                    grdEntrenadores.DataBind(); // Es necesario para que se visualicen los datos, los enlaza
                }
            }
            catch (Exception)
            {
        ...
        protected void lnkEliminar_Command(object sender, CommandEventArgs e)
        {
            ...
            try
            {
                entrenador = logica.ObtenerEntrenador(id); // Le devuelve la consulta o el registro según el ID al presionar ELIMINAR Así se sabe si existe                
                if (entrenador.Estado) // En este caso usa un estado para saber si es existe
                {
                    if (logica.EliminarEntrenador(id) > 0)
                    {
        ...
        protected void lnkModificar_Command(object sender, CommandEventArgs e)
        {
            ...
            Session["cod_entrenador"] = e.CommandArgument.ToString();

            // Redireccionamos al otro formulario (FrmClientes)

            Response.Redirect("mantenimientoEntrenador.aspx");
        }
        ...
        protected void btnBuscar_Click(object sender, EventArgs e)
        {
            try
            {
                string condicion = string.Format("NOMBRE_ENTRENADOR LIKE '%{0}%'", txtEntrenador.Text); // Se une todo en un string para hacer la condición según lo que escriba el usuario
        ...
        {
            grdEntrenadores.PageIndex = e.NewPageIndex;
            CargarListaDataSet();
        }
