...
{
    public partial class registroHora : System.Web.UI.Page
    {
        //CODE BEHIND
        ...
        {
            BLRegisHoraLabo logica = new BLRegisHoraLabo(clsConfiguracion.getConnectionString); // Se hace una instancia de la capa lógica
            DataSet DSRegisHoras;

            try
            {
                DSRegisHoras = logica.listarRegisHorasLabo(condicion, orden);
                if (DSRegisHoras != null)
                {
                    grdHorasLabo.DataSource = DSRegisHoras;
                    grdHorasLabo.DataMember = DSRegisHoras.Tables[0].TableName; // la primera tabla devuelta

                    grdHorasLabo.DataBind(); // Es necesario para que se visualicen los datos, los enlaza
                }
            }
            catch (Exception)
            {
        ...
        {
            grdHorasLabo.PageIndex = e.NewPageIndex;
            CargarListaDataSet();
        }
        ...
        protected void btnAgregar_Click(object sender, EventArgs e)
        {
            ...
            Session.Remove("cod_regisHoraLabo");

            // Redireccionamos al otro formulario:
            Response.Redirect("mantenimientoRegisHora.aspx");
        }
        ...
        protected void lnkModificar_Command(object sender, CommandEventArgs e)
        {
            ...
            Session["cod_regisHoraLabo"] = e.CommandArgument.ToString();
        ...
        protected void lnkEliminar_Command(object sender, CommandEventArgs e)
        {
            ...
            EntidadRegisHoraLabo certificacion = new EntidadRegisHoraLabo();

            try
            {
                certificacion = logica.ObtenerRegisHorasLabo(id); // Le devuelve la consulta o el registro según el ID al presionar ELIMINAR Así se sabe si existe                
                if (certificacion.Existe)
                {
                    if (logica.EliminarHorasLabo(id) > 0)
                    {

                        script = string.Format("javascript:mostrarMensaje('Horas Registradas eliminadas satisfactoriamente')");
                        ScriptManager.RegisterStartupScript(this, typeof(string), "MensajeRetorno", script, true);
